...
def my_call_back(coro, current_time):
    ...
    print(f'当前协程的返回值：{coro.result()}，协程的结束时间：{current_time}')
...
async def main():       # 其他协程的入口
    ...
    # 添加回调函数
    # task1.add_done_callback(my_call_back)     # 如果回调函数没有参数可以这样写
    task1.add_done_callback(partial(my_call_back, current_time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
